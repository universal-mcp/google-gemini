...
from universal_mcp_google_gemini.app import GeminiApiApp

@pytest.fixture
def app_instance():
    """Provides a GeminiApiApp instance for tests."""
    mock_integration = MagicMock()
    mock_integration.get_credentials.return_value = {"access_token": "dummy_access_token"}

    return GeminiApiApp(integration=mock_integration)

def test_universal_mcp_google_gemini_app_initialization(app_instance):
    """
    Test that the GeminiApiApp instance is initialized correctly with a name.
    """
    ...
    assert app_instance.name == "geminiapiapp", "GeminiApiApp instance has unexpected name."


def test_universal_mcp_google_gemini_tool_docstrings_format(app_instance):
    """
    Test that each tool method in GeminiApiApp has a well-formatted docstring,
...
def test_universal_mcp_google_gemini_tools_are_callable(app_instance):
    """
    Test that each tool method returned by list_tools in GeminiApiApp is callable.
    """
    tools = app_instance.list_tools()
    assert isinstance(tools, list), "list_tools() should return a list."
