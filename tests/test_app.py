...
@pytest.fixture
def app_instance():
    """Provides a GoogleGeminiApp instance for tests."""
    mock_integration = MagicMock()
    mock_integration.get_credentials.return_value = {"access_token": "dummy_access_token"}

    return GoogleGeminiApp(integration=mock_integration)

def test_universal_mcp_google_gemini_app_initialization(app_instance):
    """
    Test that the GoogleGeminiApp instance is initialized correctly with a name.
    """
    ...
    assert app_instance.name == "GoogleGeminiApp", "GoogleGeminiApp instance has unexpected name."


def test_universal_mcp_google_gemini_tool_docstrings_format(app_instance):
    """
    Test that each tool method in GoogleGeminiApp has a well-formatted docstring,
...
def test_universal_mcp_google_gemini_tools_are_callable(app_instance):
    """
    Test that each tool method returned by list_tools in GoogleGeminiApp is callable.
    """
    tools = app_instance.list_tools()
    assert isinstance(tools, list), "list_tools() should return a list."
